...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const middle_product_description = document.getElementById(
    "middle_product_description"
  );
  ...
  const right_bidding_updateprice = document.getElementById(
    "right-bidding-update"
  );
  ...
  function showTime() {
    ...
    document.getElementById(
      "MyClockDisplay"
    ).innerHTML = `${daysRemaining}d ${hoursRemaining}h ${minutesRemaining}m ${secondsRemaining}s`;
  ...
  try {
    ...
    middle_product_description.textContent =
    ...
    if (productdetail.data.data.userID === urlusername) {
      ...
      parseInt(getcoins.data.data) <
      parseInt(productdetail.data.data.normal_price)
    ) {
      bidbutton.disabled = true;
