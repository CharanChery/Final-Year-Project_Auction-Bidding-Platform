...
const createtask = async(req,res)=>{
    const {username,email,password,otp,data, fullname}= req.body
    if (parseInt(data, 10)===data){
        try {
            ...
            const detail = await DetailSchema.create({ username,email, password: hashedPassword,coins:0, fullname });
